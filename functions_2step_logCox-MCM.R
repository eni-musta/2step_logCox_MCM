...
# input:  xx = matrix containing the covariates X as columns, ordered with respect to the follow-up times, 
#          h = bandwidth 
# output:  w = matrix with elements w_ij=K_h(z_j-z_i)/sum_l K_h(z_l-z_i) where z_i is the standardized version of the new 1-dim covariate, the Epanechnikov kernel is used


weight_1dim = function(par,xx,h)
{
  n = dim(xx)[1]      
  m = length(par)      
  xx=par[1]+c(par[2:m]%*%t(xx))
  xx=(xx-mean(xx))/sd(xx)
...
# L(gamma)=product_i {phi(gamma,X_i)^(1-pnonp(X_i))}{[1-phi(gamma,X_i)]^pnonp(X_i)}
# input: gamma parameter, a matrix V which contains the covariates starting from third column, the nonparamtric estimator pnonp

Lik_logit_1dim = function(par,V,pnonp) 
{ 
  m=length(par)
  phi = exp(-par[1]-c(par[2:m]%*%t(V[,(3:(m+1))])))
  phi = 1/(1+phi)
...
em2 = function(Time,Status,X,Z,gamma,beta_init,emmax,eps){ 
    ...
    em2 = list(beta=beta,Survival=s,tau=convergence)
  }
...
{     
  ...
  em1 = list(b=gamma, beta= beta,survival=s,tau=convergence)
}
